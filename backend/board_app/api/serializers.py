...
class BoardDetailSerializer(serializers.ModelSerializer):
    ...
    def get_members(self, obj):
        ...
        serializer = SimpleUserSerializer(users, many=True, context=self.context)
...
class BoardSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        memberships_to_create = [BoardMembership(user=user, board=board, role="owner")]
        for user_id in member_ids:
            if user_id != user.id:
                memberships_to_create.append(
                    BoardMembership(user_id=user_id, board=board, role="member")
                )
    ...
    def update(self, instance, validated_data):
        ...
        if "members" in validated_data:
            ...
            if members_to_remove:
                BoardMembership.objects.filter(
                    board=instance, user_id__in=members_to_remove
                ).delete()
            if members_to_add:
                new_memberships = [
                    BoardMembership(user_id=user_id, board=instance, role="member")
